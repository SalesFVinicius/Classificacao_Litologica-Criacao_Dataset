# -*- coding: utf-8 -*-
"""
Created on Sun Jun  8 12:26:10 2025
...
import shutil
import random 
from Other.Dataset_Creation_TP import Dataset_Creation
from Other.Train_Test_Split import Train_Test_Split

class Clip_Patches_Img(Train_Test_Split):
    
    def split_image_center(self,tile_width, tile_height,cont = 0,remove_classe=None): 
        
            print('Hiper')
            file_gt = self.select_file()
            print('Classe')
            file = self.select_file()
            print('Imagem de Pontos Selecionados')
            file_pontos = self.select_file()

            
            points = self.position_class(file_pontos,remove_classe)
                ...
                for classe , linha_central, coluna_central in amostra:
                    # try :
                        with rio.open(file) as inds:
                            meta = inds.meta.copy()
                            window, transform = self.criar_janela_centrada(linha_central, coluna_central, tile_height, tile_width, inds)
                            ...
                            if tile_width == window.width and window.height == tile_height:
                                    ...
                                    if proporcao <0.1 :
                                        ...
                                        with rio.open(file_gt) as inds_gt:
                                            ...
                                            window_gt, transform_gt = self.criar_janela_centrada(linha_central, coluna_central, tile_height, tile_width, inds_gt)
    ...
    def criar_janela_centrada(self,linha_central, coluna_central, tile_height, tile_width, ds):
